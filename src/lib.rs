...
impl PyBlockNms {
    fn run(&mut self, data: PyReadonlyArray2<u16>, dist_blocks: i32, total: usize) -> PyResult<Vec<XY>> {
        ...
        for by in 0..self.blocks_h {
            ...
            for bx in 0..self.blocks_w {
                ...
                let (best_v, best_xy) = find_block_max(&iw, x0, x1, y0, y1);
                self.max_vals[block_idx] = best_v;
        ...
        self.order.sort_by(|&a, &b| {
            let va = self.max_vals[a];
            let vb = self.max_vals[b];
            vb.cmp(&va)
        });
        ...
        for &block_idx in &self.order {
            ...
            out.push(self.max_xy[block_idx]);
            ...
            let y_min = (by - dist_blocks).max(0);
            let x_min = (bx - dist_blocks).max(0);
            let y_max = (by + dist_blocks).min(self.blocks_h - 1);
            let x_max = (bx + dist_blocks).min(self.blocks_w - 1);
    ...
    #[new]
    fn new(square: i32, height: i32, width: i32) -> Self {
        ...
        Self { 
            square, height, width, blocks_w, blocks_h,
            suppressed, order, max_vals, max_xy,
        }
    }
}
